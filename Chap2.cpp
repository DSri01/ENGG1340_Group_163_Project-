...
bool Chap2(string *Pname, int *Phitp, int *Phealth, int *Psol, int *ChoiceC1){//Funtion to implement chapter 2 of the game. It uses different functions from different cpp files and nested if-else statements to do so. If the game is lost then this function returns true, else it returns false
  ...
  *ChapFiles = "Chapters/Chapter2c2.txt";
  PrintChapLines(ChapFiles, Lines);//Printing lines from chapter files in Chapters directory
  cout<<"Would you follow the sound and trace its origin or would you rather continue on your path ignoring the screams of pain?"<<endl;
  cout<<"Enter 1 or 2 based on your choice."<<endl;
  *ChoiceNum = 2;
  Choice(Choice1, ChoiceNum);//getting choice of the player
  if(*Choice1 == 1){//nested if-else statement to advance game based on player's choice
    cout<<"You try to follow the sound and find that there is a person stuck inside a thick bushel with thorns piercing his entire body."<<endl;
    cout<<"Blood and a thick green sap are oozing out of all the places the bush was touching."<<endl;
    cout<<"You go forward to help him, but he warns you about the poison ivy bushes."<<endl;
    cout<<"Will you help him anyway?"<<endl;
    cout<<"Enter 1 to help or 2 to leave him."<<endl;
    Choice(Choice1, ChoiceNum);//getting choice of the player
    if(*Choice1 == 1){
      cout<<"You try to take the man out. The man asks you to not do so but you still keep going forward. You get hurt too, but the man chants a spell on you so that your health does not fall below 1."<<endl;
      cout<<"Health points left = 1."<<endl;
      *Lines = 12;
      ...
      *Phealth = *Phealth + 2;
      ...
      cout<<"You take his advice and ask for his forgiveness for not being able to help. He accepts it and tells you a way out of the forest. You go on while the man screams with pain."<<endl;
      cout<<"But as you turned to see him one last time, you get pierced by a thorn of the bush too."<<endl;
      cout<<"Health -3"<<endl;
      *Phealth = *Phealth - 3;
